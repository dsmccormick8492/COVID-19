...
la_daily_gb = states_gb.get_group('LA')
la_daily_df = la_daily_gb.sort_values(by=['date'], ignore_index=True)


#%% plots!
plot_daily_data(us_daily_df, title_str='All US')
plot_daily_data(ma_daily_df, title_str='Massachusetts')
plot_daily_data(ny_daily_df, title_str='New York')
plot_daily_data(nj_daily_df, title_str='New Jersey')
plot_daily_data(ca_daily_df, title_str='California')
plot_daily_data(tx_daily_df, title_str='Texas')
plot_daily_data(la_daily_df, title_str='Louisiana')

#%% some regression tests

df = la_daily_df[['datetime', 'death']].copy().dropna().reset_index(drop=True)
